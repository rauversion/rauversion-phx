defmodule Rauversion.EventTicketsTest do
  ...
  describe "event_tickets" do
    ...
    @invalid_attrs %{early_bird_price: nil, price: nil, qty: nil, selling_end: nil, selling_start: nil, settings: nil, short_description: nil, standard_price: nil, title: nil}
    ...
    test "create_event_ticket/1 with valid data creates a event_ticket" do
      valid_attrs = %{early_bird_price: "120.5", price: "120.5", qty: 42, selling_end: ~U[2022-08-26 04:47:00Z], selling_start: ~U[2022-08-26 04:47:00Z], settings: %{}, short_description: "some short_description", standard_price: "120.5", title: "some title"}
    ...
    test "update_event_ticket/2 with valid data updates the event_ticket" do
      event_ticket = event_ticket_fixture()
      update_attrs = %{early_bird_price: "456.7", price: "456.7", qty: 43, selling_end: ~U[2022-08-27 04:47:00Z], selling_start: ~U[2022-08-27 04:47:00Z], settings: %{}, short_description: "some updated short_description", standard_price: "456.7", title: "some updated title"}

      assert {:ok, %EventTicket{} = event_ticket} = EventTickets.update_event_ticket(event_ticket, update_attrs)
    ...
    test "update_event_ticket/2 with invalid data returns error changeset" do
      event_ticket = event_ticket_fixture()
      assert {:error, %Ecto.Changeset{}} = EventTickets.update_event_ticket(event_ticket, @invalid_attrs)
