defmodule RauversionWeb.MyPurchasesLive.Index do
  use RauversionWeb, :live_view

  defp get_items() do
    [
      %{href: "/purchases/tickets", name: "Tickets"},
      %{href: "/purchases/music", name: "Music"}
    ]
  end
  ...
  defp icon_for(_kind = "Tickets") do
    ...
    ~H"""
    <svg class="h-10 w-10 flex-shrink-0 rounded-full text-gray-800 dark:text-gray-100" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  ...
                  <div class="flex w-full items-center justify-between space-x-6 p-6">
                    <div class="flex-1 truncate">
                      <div class="flex items-center space-x-3">
                        <h3 class="truncate text-sm font-medium text-gray-900 dark:text-gray-100"><%= item[:name] %></h3>
                      ...
                      <p class="mt-1 truncate text-sm text-gray-500 dark:text-gray-300"><%= gettext("You purchased %{name}", name: item[:name]) %>  </p>
                    </div>
