defmodule RauversionWeb.TrackLive.SharerComponent do
  ...
  def render(assigns) do
    ~H"""
      <ul class="table clear-both p-0 m-0">

        <li class="float-left my-0 mr-2 ml-0">
          <a
            type="button"
            class="text-blue-500 flex space-x-2"
            tabindex="0"
            href={"https://twitter.com/intent/tweet?#{Routes.track_show_url(@socket, :show, @track,
            utm_source: "twitter",
            utm_medium: "post",
            utm_campaign: "social_sharing",
            text: "#{@track.title} by #{@track.user.username}"
          )}"}
            target="_blank"
        ...
        <li class="float-left my-0 mr-2 ml-0">
          <a
            ...
            <span><%= gettext "Share on Facebook" %></span>
            </a>
        </li>
      </ul>
    """
