defmodule RauversionWeb.EventsLive.NavBar do
  ...
  def menu_items(event) do
    [
      ...
      %{
        to: "/events/#{event.slug}/edit/recordings",
  ...
  def render(assigns) do
    ...
    >
      <div class="hidden xl:flex flex-shrink-0 h-16 px-6 border-b border-blue-gray-600 flex items-center justify-between">
        ...
        <%= if @event.id do %>
          <%= live_redirect to: Routes.events_show_path(assigns.socket, :show, @event.slug),
            ...
            <svg
              ...
              x-description="Heroicon name: mini/arrow-top-right-on-square"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden="true"
